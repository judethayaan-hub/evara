...
function CustomerDashboard({ user, token, onGoToBookings, onGoToExplore }) {
  const [bookings, setBookings] = useState([]);
  const [loading, setLoading] = useState(true);
    ...
    <>
      ...
      <div className="evara-shell">
        ...
        <main className="evara-main">
          ...
          {tab === "dashboard" && (user ? <CustomerDashboard user={user} token={token} onGoToBookings={() => setTab("bookings")} onGoToExplore={() => setTab("explore")}/> : <ExplorePage user={user} token={token} onVendorSelect={setSelectedVendor} onShowAuth={() => setShowAuth(true)}/>)}
